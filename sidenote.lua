...
function Doc(body, metadata, variables)
  ...
  add(body)
  if #notes > 0 then
    add('<footer><hr class="footnotes"><ol class="footnotes">')
    for _,note in pairs(notes) do
      local s = string.gsub(note[2],
          '(.*)</', '%1 <a href="#fnref' .. note[1] ..  '"' ..
          ' class="fnref">&#8617;</a></')
      add('<li id="fn' .. note[1] .. '">' .. s .. '</li>')
    end
    add('</ol></footer>')
  end
...
function Note(s)
  ...
  return '<a id="fnref' .. num .. '" class="fnref" href="#fn' .. num ..
            '"><sup>' .. num .. '</sup></a>'
end
...
function Para(s)
  ...
  if footnotes ~= "" then
    footnotes = '<aside class="sidenote"><ol start="' .. start .. '">' .. footnotes .. '</ol></aside>'
  end
  return footnotes ..
         '<p>' .. s .. '</p>'
end
...
function prismClass(cls)
  if string.find(cls, "haskell") then
...
function CodeBlock(s, attr)
  ...
  if attr.class and string.match(' ' .. attr.class .. ' ',' dot ') then
    ...
    return "<pre><code class=\"" .. prismClass(attr.class) .. "\">" .. escape(s) ..
           "</code></pre>"
  end
end
